...
import { getMembers, getResponses, getSupabaseStatus, exportAll, importAll, seedLocal, clearAll, exportExcel, exportCSV } from '../services/api'
...
export default function Admin() {
  ...
  useEffect(() => {
    if (!authed) return
    Promise.all([getMembers(), getResponses(), getSupabaseStatus()]).then(([m, r, s]) => {
      setMembers(m || [])
      setResponses(r || [])
      setSupabase(s || null)
    ...
    <div className="p-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2 items-center">
          <div className={`px-2 py-1 rounded text-sm ${supabase?.ok ? 'bg-green-700' : 'bg-neutral-700'}`}>
            Banco local: {supabase?.ok ? 'OK' : 'Indisponível'}
          </div>
          <button className="px-3 py-2 rounded bg-blood text-white" onClick={() => { exportAll().catch(()=>{}) }}>
            Exportar JSON
          </button>
          <button className="px-3 py-2 rounded bg-neutral-700 text-white" onClick={() => { exportCSV().catch(()=>{}) }}>
            Exportar CSV
          </button>
          <button className="px-3 py-2 rounded bg-neutral-700 text-white" onClick={() => { exportExcel().catch(()=>{}) }}>
            Exportar Excel
          </button>
          <label className="px-3 py-2 rounded bg-neutral-700 text-white cursor-pointer">
            Importar JSON
